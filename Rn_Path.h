...
void ApplyPerspective2Positions( const int nTotal ) {
   const int first  = FollowerConstants::globalWhichComponentsToPlot.first;
   const int second = FollowerConstants::globalWhichComponentsToPlot.second;
   const size_t nProbes= (size_t)m_reducedList.size( );
   double xMax = -DBL_MAX;
   double yMax = -DBL_MAX;
   double xMin = DBL_MAX;
   double yMin = DBL_MAX;
