...
private:
      ...
      /*-------------------------------------------------------------------------------------*/
   {
      ...
      std::string s = ToString( "   <polyline fill=\"none\" stroke=\"black\" stroke-width=\"", SVG_DistancePlotConstants::globalDeloneDataLineStrokeWidth, "\" points=\" " );

      std::list<double>::const_iterator it = distances.begin( );
      for (int i = 1; i <= xmax; ++i, ++it)
         s += ToString( double( i )*xscale, "," ) +
         ToString( ((*it)-minimumDistance) *yscale, " " );
      ...
      /*-------------------------------------------------------------------------------------*/
   {
      return(
         ...
         + ToString( " stroke-width=\"", SVG_DistancePlotConstants::globalDataAxisWidth, "\" " )
         + ToString( " points=\" 600,0 0,0 0,", (alY.m_upperLimit - minimumDistance)*yscale, "\" />" )
         + "\n<!-- -->\n");
   }
      ...
      /*-------------------------------------------------------------------------------------*/
   {
      ...
      if (!glitches.empty( )) {
         ...
         for( size_t iglitch = 0; iglitch < glitches.size( ); ++iglitch)
         {
...
//      return std::list<std::string>();
      ...
      /*-------------------------------------------------------------------------------------*/
   {
      ...
      const std::string fileName( ToString( groupedDataFileName ) + "_DIST.txt" );
      std::ofstream folOut;
   ...
   void InsertVectorAndDifferenceData( const std::list<double>& distances, const std::list<double>& Delonedistances )
   {
      ...
      {
         m_svg.push_back( ToString( *iProbe  )
            + "   (" + ToString(*iDist1)
            + ToString(",  ", *iDist2, ")")
            + ToString( "   ", m_follow.GetBoundaryString( count ) ) );
      }
   ...
   {
      return(
         ToString( "<line fill=\"none\" stroke-width=\"", SVG_DistancePlotConstants::globalG6DataLineStrokeWidth, "\"" )
         + ToString( " boundary-string=\"", bound, "\" " )
         + ToString( " stroke=\"", m_colorMap( bound ), "\"" )
         + ToString( " x1=\"", x1, "\"" )
         + ToString( " y1=\"", y1, "\"" )
         + ToString( " x2=\"", x2, "\"" )
         + ToString( " y2=\"", y2, "\" />" ));
...
private: // don't implement
   ...
   static std::string DrawOneY_AxisTicMark( const double y ) {
      return(
         ToString( "<line fill=\"none\" stroke=\"black\"" )
         + ToString( " stroke-width=\"", SVG_DistancePlotConstants::globalDataAxisWidth, "\"" )
         + ToString( " x1=\"0.0\"" )
         + ToString( " y1=\"", y, "\"" )
         + ToString( " x2=\"", -SVG_DistancePlotConstants::globalY_AxisTicMarkLength, "\"" )
         + ToString( " y2=\"", y, "\" />" ));
   }
   ...
   static std::string DrawOneY_AxisLabel( const double y, const double yscale, const double minimumDistance ) {
      return(
         ...
         + ToString( " x=\"", -SVG_DistancePlotConstants::globalY_AxisTicMarkLength - 5, "\"" )
         + ToString( " y=\"", -y + 10, "\" >" )
         + ToString( y / yscale + minimumDistance, "</text>" ));
   }
