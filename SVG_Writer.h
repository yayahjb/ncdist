...
private:
   ...
   /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
   static double Progress( const size_t position, const size_t totalCount ) {
      return(800 * double( position + 1 ) / double( std::max( size_t(1), totalCount ) ));
   }
   ...
   std::vector<int> DetermineWhichFramesToReject( const std::vector<int>& rejectedFrames ) const {
      ...
      for( size_t i = 1; i != rejectedFrames.size( ) - 1; ++i)
   ...
   std::string GetWhichToPlot( const std::pair<int, int>& which2plot ) {
      return(ToString( "Plotting g" ) + ToString( which2plot.second + 1 ) + ToString( " vs. g" ) + ToString( which2plot.first + 1 ));
   }
   ...
   /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
   std::string DrawOverallProgressBar( const int counter, const size_t totalListCount ) const {
      const std::string strEndY_Progress( ToString( 100 + Progress( counter - 1, totalListCount ) ) );
   ...
   /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
   std::string DrawOverallProgressBarBase( const size_t totalListCount ) const
      /*-------------------------------------------------------------------------------------*/
   {
      std::string str = std::string( "<!-- Overall Progress Bar (base) -->\n" ) + std::string( "<g>\n" );
      std::string previousSegmentEnd = "100";

      for( size_t i = 0; i < totalListCount; ++i)
      {
         const std::string strEndY_ProgressSegment = ToString( 100 + Progress( i, totalListCount ) );
   ...
   std::string DrawFirstGlitch( ) const {
      ...
      if (!glitchList.empty( ))
      {
         ...
         s += "<text x=\"300\" y=\"160\"  font-family=\"sans-serif\" font-size=\"20\" > Glitch  " + ToString( vG6 ) + " (" + ToString( dist1 ) + " )</text>\n";
         vG6 = (*glitchList.begin( )).GetGlitchElement2( ).GetVector( );
         s += "<text x=\"300\" y=\"185\"  font-family=\"sans-serif\" font-size=\"20\" > Glitch  " + ToString( vG6 ) + " (" + ToString( dist2 ) + ")</text>\n";
      }
      ...
      /*-------------------------------------------------------------------------------------*/
   {
      ...
      for( size_t i = 0; i < rejects.size( ); i += 2)
   ...
   std::string ScaleToGraph( const size_t n, const size_t totalItemCount ) const {
      ...
      return(ToString( placement ));
   }


   /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
   std::string DrawSingleRejectedSegment( const size_t reject1, const size_t reject2 ) const
      /*-------------------------------------------------------------------------------------*/
   {
      if (reject1 >= 0 && reject1 != reject2){
         ...
         str += "   <line x1=\"970\" y1=\"" +
            ...
            ScaleToGraph( reject2 + size_t(1), FollowerConstants::globalFramesPerSegment ) +
   ...
   void SVG_HeaderFrameDescription( const int startingVectorCounter ) {
      if (startingVectorCounter >= 0) {
         m_svg.push_back( " <text x=\"280\"  y=\"30\"  font-family=\"sans-serif\" font-size=\"20\" > Trial vector = " + ToString( startingVectorCounter ) + "  </text> " );
         m_svg.push_back( " <text x=\"700\" y=\"30\" font-family=\"sans-serif\" font-size=\"20\">Wall time passed: "
            + ToString( m_follow.GetTime2ComputerFrame( ) )
            + " msec.</text>" );
      }
   }
   ...
   /*+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
   std::string OnePieceOfProgressBar( const size_t i, const size_t totalListCount, const std::string& previousSegmentEnd ) const
   {
      const std::string strEndY_ProgressSegment = ToString( 100 + Progress( i, totalListCount ) );
