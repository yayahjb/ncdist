...
int main(int argc, char ** argv) {
    ...
    if (arg1 == "--help" || arg1 == "-h") {
        std::cerr 
		<< "Usage: cs6dist_map [--help|-h] print this message and exit" 
		<< std::endl;
        std::cerr 
		<< "       cs6dist_map with no arguments, write distance matrix to cout" 
		<< std::endl;
        std::cerr 
    ...
    for (ii=0; ii < inputprims.size(); ii++) {
        for (jj=11+1; jj < inputprims.size(); jj++) {
            std::cout <<" "<<dmat[ii][jj];
        }
        std::cout << std::endl;
    }
